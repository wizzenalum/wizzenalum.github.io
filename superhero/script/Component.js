...
let setSearchComponent = function(state){
    console.log("SEARCH")
    let list = createList(state.searchList,state.currentWindow);
    if(!list) list = `<h2 style="margin:auto">type any SuperHero name</h2>`;
    let htmlText = `
            <main>
                <div class="search">
...
let setHeroComponent = function({hero}){
    let appearance = "",biography = '',powerstats="",relations="";
    for(let property in hero.appearance){
        if(hero.appearance[property].constructor===String && hero.appearance[property].length>2) {
    ...
    for(let property in hero.biography){
        if(hero.biography[property].constructor===String && hero.biography[property].length>2) {
            biography +=`<li>${property} : ${hero.biography[property]}</li>`
        }
    }
    for(let property in hero.relations){
        if(hero.connections[property].constructor===String && hero.connections[property].length>2) {
